...
public final class TextConstants {
    private static Map<Locale, TextConstants> cache = new HashMap<>();
    public static TextConstants getForLocale(Locale locale) {
        if (!cache.containsKey(locale)) {
            ResourceBundle bundle = ResourceBundle.getBundle("stringConstants", locale);
            cache.put(locale, new TextConstants(bundle));
        }
